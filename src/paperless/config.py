...
class AIConfig(BaseConfig):
    ...
    vector_store_password: str | None = dataclasses.field(init=False)
    vector_store_database: str | None = dataclasses.field(init=False)
    ...
    def __post_init__(self) -> None:
        ...
        self.ai_system_prompt = getattr(
            ...
            "ai_system_prompt",
            None,
        ) or getattr(settings, "AI_SYSTEM_PROMPT", None)
        ...
        self.vector_store_backend = getattr(
            ...
            None,
        ) or getattr(settings, "VECTOR_STORE_BACKEND", "auto")
        self.vector_store_host = getattr(
            app_config,
            "vector_store_host",
            None,
        ) or getattr(settings, "VECTOR_STORE_HOST", None)
        self.vector_store_port = getattr(
            app_config,
            "vector_store_port",
            None,
        ) or getattr(settings, "VECTOR_STORE_PORT", None)
        self.vector_store_user = getattr(
            app_config,
            "vector_store_user",
            None,
        ) or getattr(settings, "VECTOR_STORE_USER", None)
        self.vector_store_password = getattr(
            app_config,
            "vector_store_password",
            None,
        ) or getattr(settings, "VECTOR_STORE_PASSWORD", None)
        self.vector_store_database = getattr(
            app_config,
            "vector_store_database",
            None,
        ) or getattr(settings, "VECTOR_STORE_DATABASE", None)
