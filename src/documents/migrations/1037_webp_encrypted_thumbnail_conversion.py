    ...
    with tempfile.TemporaryDirectory() as tempdir:
        ...
        if len(list(Path(settings.THUMBNAIL_DIR).glob("*.png.gpg"))) > 0:
            ...
            if work_packages:
                ...
                # primarily subprocess-bound (GPG and ImageMagick), so
                # threads are efficient enough and avoid issues with
                # Django's app registry in spawned processes (especially
                # on macOS).
                with ThreadPool(
